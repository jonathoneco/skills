---
name: teammate
description: Spawn an addressable team-mode agent that chats with the user directly for one iteration. Use when a caller (`/next-afk`, `/next-hitl`, etc.) needs an iteration that stays addressable for back-and-forth user sign-offs or mid-flight chime-in.
---

# Teammate

You are spawning an addressable teammate for one iteration of work, surfacing its name to the user, then tearing the team down on close. You own spawn, engagement-contract surfacing, and tear-down — nothing else.
...
- Fall back to a one-shot `Agent` call (no addressability, no chat-back) and tell the caller you fell back.
